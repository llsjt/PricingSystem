...
import { applyDecision, cancelPricingTask, createPricingTask, getPricingTaskDetail, getPricingTaskLogs, getPricingTaskStreamUrl, getTaskComparison, type AgentCardContent, type DecisionComparisonItem, type DecisionLogItem, type PricingAgentCode, type PricingTaskStatus, type PricingTaskStreamMessage } from '../api/decision'
...
const loadDetail = async (id: number) => { const res = await getPricingTaskDetail(id) as ApiResponse<any>; if (res.code === 200 && res.data) { state.taskStatus = (res.data.taskStatus || 'RUNNING') as PricingTaskStatus; state.finalPrice = res.data.finalPrice != null ? Number(res.data.finalPrice) : null; state.strategy = String(res.data.strategy || ''); state.finalSummary = String(res.data.finalSummary || '') } }
const loadLogs = async (id: number) => { const res = await getPricingTaskLogs(id) as ApiResponse<DecisionLogItem[]>; return res.code === 200 && Array.isArray(res.data) ? res.data : [] }
const loadComparison = async (id: number) => { const res = await getTaskComparison(id) as ApiResponse<DecisionComparisonItem[]>; comparisonData.value = res.code === 200 && Array.isArray(res.data) ? res.data : []; archiveReportSummary.value = comparisonData.value.find((row) => String(row.resultSummary || '').trim())?.resultSummary || '' }
const loadSnapshot = async (id: number) => { await loadDetail(id); let logs = await loadLogs(id); if (isTerminal(state.taskStatus) && logs.length < agents.length) { for (let i = 0; i < 5; i += 1) { await new Promise((r) => setTimeout(r, 250)); const next = await loadLogs(id); if (next.length >= logs.length) logs = next; if (logs.length >= agents.length) break } } const cards = emptyCards(); logs.sort((a, b) => Number(a.displayOrder || a.runOrder || 0) - Number(b.displayOrder || b.runOrder || 0)).forEach((log) => { const code = String(log.agentCode || '') as PricingAgentCode; if (code in cards) cards[code] = normalizeCard({ thinking: String(log.thinking || log.outputSummary || ''), evidence: log.evidence || [], suggestion: log.suggestion || {}, reasonWhy: log.reasonWhy || null }) }); state.cards = cards; await loadComparison(id) }

const handleStream = async (payload: PricingTaskStreamMessage) => { if (payload.type === 'task_started') state.taskStatus = (payload.status || 'RUNNING') as PricingTaskStatus; if (payload.type === 'agent_card') { const code = payload.agentCode as PricingAgentCode; if (code in state.cards) state.cards[code] = normalizeCard(payload.card) } if (payload.type === 'task_completed') { state.taskStatus = (payload.status || 'COMPLETED') as PricingTaskStatus; if (taskId.value) await loadSnapshot(taskId.value); stopRealtime(); ElMessage[state.taskStatus === 'MANUAL_REVIEW' ? 'warning' : 'success'](state.taskStatus === 'MANUAL_REVIEW' ? '任务已完成，当前结果需要人工审核' : '智能决策已完成，可以查看结果报告') } if (payload.type === 'task_failed') { state.taskStatus = (payload.status || 'FAILED') as PricingTaskStatus; state.errorMessage = sanitizeErrorMessage(payload.message, state.taskStatus === 'CANCELLED' ? '任务已取消' : '任务执行失败'); if (taskId.value) await loadSnapshot(taskId.value); stopRealtime(); ElMessage[state.taskStatus === 'CANCELLED' || state.taskStatus === 'MANUAL_REVIEW' ? 'warning' : 'error'](state.errorMessage) } }

const startPolling = () => { if (pollTimer || !taskId.value) return; pollTimer = setInterval(async () => { if (!taskId.value) return; if (!isRunning(state.taskStatus)) { stopRealtime(); return } await loadDetail(taskId.value); if (isTerminal(state.taskStatus)) { await loadSnapshot(taskId.value); stopRealtime() } }, 2000) }
