...
class BizProduct(Base):
    ...
    click_rate = Column(DECIMAL(5, 4), default=0.0000)
    conversion_rate = Column(DECIMAL(5, 4), default=0.0000)
...
class BizSalesDaily(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    product_id = Column(Integer, ForeignKey("biz_product.id"), nullable=False, index=True)
    stat_date = Column(Date, nullable=False, index=True)
    daily_sales = Column(Integer, default=0)
    daily_revenue = Column(DECIMAL(12, 2), default=0.00)
    created_at = Column(DateTime, default=datetime.utcnow)
...
class DecResult(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    task_id = Column(Integer, ForeignKey("dec_task.id"), nullable=False, index=True)
