...
CREATE TABLE agent_run_log (
    ...
    thought_content TEXT DEFAULT NULL COMMENT '思考内容',
    thinking_summary TEXT DEFAULT NULL COMMENT '思考过程摘要',
    evidence_json JSON DEFAULT NULL COMMENT '依据(JSON数组)',
    suggestion_json JSON DEFAULT NULL COMMENT '建议(JSON对象)',
    final_reason TEXT DEFAULT NULL COMMENT '最终建议原因(经理Agent)',
    display_order INT DEFAULT NULL COMMENT '卡片展示顺序(1-4)',
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '记录时间',
    KEY idx_task_id (task_id),
    KEY idx_task_order (task_id, speak_order),
    KEY idx_task_display_order (task_id, display_order),
