...
在数据管理页可进行商品检索、平台与状态筛选、批量定价入口触发，以及查看商品价格、库存和基础经营表现。

### 2. 单商品智能定价决策流

![](./docs/images/readme/pricing-lab-agents.png)

智能定价页按流程展示多智能体的分析摘要、关键依据和建议结果，用于承接单商品任务从启动到结果查看的主链路。

### 3. 批量定价进度与结果汇总

![](./docs/images/readme/pricing-batch-progress.png)

批量定价进度页集中展示批次状态、人工审核数量、子任务建议价与执行策略，便于回看批次执行结果。

## 文档入口

- [API文档](./docs/API文档.md)：按模块整理的可读版接口文档，包含请求路径、方法、入参和返回值
...
- 市场情报来自本地天猫 CSV 竞品索引生成的 SQLite 数据，Python Worker 按类目和标题匹配竞品样本；当前不是实时爬取或实时调用第三方平台数据接口。
