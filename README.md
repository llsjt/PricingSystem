...
- **可控工具调用**：Python Worker 在预计算基线之上支持职责隔离的 ReAct-style function calling，并将工具审计写入现有 Agent 日志。
...
- 各 Agent 只能调用职责范围内的工具，工具调用有轮次、次数和超时上限，调用摘要随 `agent_run_log.raw_output_json` 留痕。
